OBJS = out/kernel.o out/common.o out/monitor.o out/main.o out/gdtidt.o out/nasmfunc.o out/isr.o out/interrupt.o \
	 ...
	 out/shutdown.o  out/net.o out/screen.o out/execute.o out/log.o out/dma.o out/audio.o out/pit.o out/fat32.o out/sb16.o \
	 out/usb.o out/usb_ohci.o out/beep.o out/ac97.o out/math.o
...
out/%.o : drivers/audio/%.c
	i686-elf-gcc -c -I include -O0 -fno-builtin -fno-stack-protector -o out/$*.o drivers/audio/$*.c


out/%.o : drivers/audio/%.asm
	nasm -f elf -o out/$*.o drivers/audio/$*.asm

out/%.o : drivers/net/%.c
	i686-elf-gcc -c -I include -O0 -fno-builtin -fno-stack-protector -o out/$*.o drivers/net/$*.c


out/%.o : drivers/net/%.asm
	nasm -f elf -o out/$*.o drivers/net/$*.asm

out/%.o : drivers/usb/%.c
	i686-elf-gcc -c -I include -O0 -fno-builtin -fno-stack-protector -o out/$*.o drivers/usb/$*.c



out/%.o : drivers/usb/%.asm
	nasm -f elf -o out/$*.o drivers/usb/$*.asm
...
hd.img : out/boot.bin out/loader.bin out/kernel.bin $(APPS)
	ftimage hd.img -size 90 -bs out/boot.bin
...
iso : hd.img
	mkisofs -o os.iso -b hd.img -no-emul-boot -boot-load-size 4 -boot-info-table . 
