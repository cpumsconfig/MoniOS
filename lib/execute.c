...
int cmd_cd(const char *path) {
    ...
    if (strcmp(path, "..") == 0) {
        ...
        //printf("Debug: Changed to parent directory, current_path = '");
        //printf(current_path);
        //printf("'\n");
    ...
    // 调试信息：显示提取的目录名
    //printf("Debug: Extracted dirname = '");
    //printf(dirname);
    //printf("'\n");
...
static int16_t tone[48000 * 2]; // 1秒 48kHz 立体声

static void gen_tone(){
    // 简单生成左右不同频率的正弦，范围 int16_t
    const double pi = 3.141592653589793;
    double fL = 440.0, fR = 660.0, sr = 48000.0;
    for(uint32_t i=0;i<48000;i++){
        double sL = sin(2*pi*fL*i/sr);
        double sR = sin(2*pi*fR*i/sr);
        tone[i*2+0] = (int16_t)(sL * 30000);
        tone[i*2+1] = (int16_t)(sR * 30000);
    }
}
...
int demo(){
    ...
    if(ac97_init()!=0){
        // VM 没有 AC'97（或被禁用）时可 fallback 蜂鸣器
        beep_play(440); beep_delay(300); beep_stop();
        return -1;
    }

    // 播放 1 秒 48k 立体声
    audio_play(48000, 48000);

    // 空转等待播放完成（最小实现，没有中断回调）
    for(volatile uint32_t t=0;t<100000000;t++) __asm__ volatile("nop");
